...
public interface GameStatsDAO {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    long insertGameStatsAndGetId(GameStats stats);
    ...
    @Query("SELECT * FROM game_stats WHERE team1_name = :thisTeam1 AND team2_name = :thisTeam2 ORDER BY game_date DESC")
    LiveData<List<GameStats>> getGameStatsByTeamMatchup(String thisTeam1, String thisTeam2);
